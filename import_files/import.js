...
var sequelize = new Sequelize("welist", "root", "root", {
  host: "localhost",
  dialect: "mysql",
  pool: {
    max: 5,
...
fs.readFile("users_5_22.csv", "utf8", function(error,data) {
    if (!error) {

      // split the file data into an array of lines of text
      var userRows = data.split("\r\n");


      for (var i = 1; i < userRows.length; i++) {

         var rowFields = parse(userRows[i]);
        ...
        //console.log(rowFields[1]);
         ...
         //if (Number(rowFields[1]) > 100) {
            // console.log(rowFields[0]);
            // console.log(rowFields[1]);
            // console.log(rowFields[2]);
            // console.log(rowFields[3]);

            User.create({

                userid: rowFields[0],
                first_name: rowFields[1],
                last_name: rowFields[2],
                username: rowFields[3],
